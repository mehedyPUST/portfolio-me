...
export default function Navbar({ items }) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50">
      {/* ১. থিম ভেরিয়েবল ইন্টিগ্রেশন: globals.css এর গ্লাস ইফেক্ট থিমিং ব্যবহার করা হয়েছে */}
      <div
        className="absolute inset-0 transition-all duration-300"
        style={{
          background: "var(--surface-glass)",
          backdropFilter: "blur(12px)",
          borderBottom: "1px solid var(--overlay-light-b)",
          boxShadow: scrolled ? "0 10px 30px -10px rgba(15, 23, 42, 0.08)" : "none"
        }}
      />

      {/* সাবল গ্রেডিয়েন্ট ওভারলে */}
      <div className="absolute inset-0 bg-gradient-to-r from-orange-500/5 via-transparent to-pink-500/5 pointer-events-none" />

      {/* বটম বর্ডার লাইটিং */}
      <div className="absolute bottom-0 left-0 right-0 h-px bg-gradient-to-r from-transparent via-orange-500/20 to-transparent pointer-events-none" />

      <div className="relative mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 transition-all duration-500 sm:px-6 sm:py-4 lg:px-8 lg:py-5">
        ...
        <div className="flex items-center gap-4 md:gap-6">
          ...
          >
            <span className="relative flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br from-orange-500/10 to-pink-500/10 transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg group-hover:shadow-orange-500/25 sm:h-14 sm:w-14">
              {/* ২. Next.js ইমেজ বাগ ফিক্স: preload={true} পরিবর্তন করে priority দেওয়া হয়েছে */}
              <Image
                src="/icon.png"
                alt="Mehedy Hasan"
                width={60}
          ...
          {items.map((item, index) => (
            <div key={`${item.label}-${item.href}-${index}`} className="relative">
              <a
                ...
                onMouseEnter={() => setHoveredIndex(index)}
                onMouseLeave={() => setHoveredIndex(null)}
                className={`relative block px-4 py-2 text-lg font-semibold transition-all duration-300 ${activeIndex === index
        ...
        <div className="flex items-center gap-3">
          ...
          >
            <span className="absolute left-1/2 top-1/2 flex w-5 -translate-x-1/2 -translate-y-1/2 flex-col gap-1.5">
              <span className={`h-0.5 w-5 rounded-full bg-slate-700 transition-all duration-300 dark:bg-white ${open ? "rotate-45 translate-y-2" : ""}`} />
              <span className={`h-0.5 w-5 rounded-full bg-slate-700 transition-all duration-300 dark:bg-white ${open ? "opacity-0" : ""}`} />
              <span className={`h-0.5 w-5 rounded-full bg-slate-700 transition-all duration-300 dark:bg-white ${open ? "-rotate-45 -translate-y-2" : ""}`} />
            </span>
          </button>
        </div>
      ...
      {open && (
        <div className="relative mx-auto max-w-7xl px-4 pb-4 sm:px-6 lg:px-8">
          <nav className="grid gap-2 rounded-2xl p-3 shadow-2xl backdrop-blur-xl sm:rounded-3xl sm:p-4 lg:hidden"
            style={{ background: "var(--surface-glass)", border: "1px solid var(--overlay-light-b)" }}>
            {items.map((item, index) => (
              <a
                key={`${item.label}-${item.href}-${index}`}
                href={item.href}
